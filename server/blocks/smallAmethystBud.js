/**
 * SmallAmethystBud - First growth stage of amethyst crystals
...
class SmallAmethystBud extends Block {
  /**
   * Create a new SmallAmethystBud block
   * @param {Object} options - Block options
   */
  constructor(options = {}) {
    super(options);
    this.type = 'small_amethyst_bud';
    ...
    this.drops = [{ id: 'amethyst_shard', count: 1, probability: 0.2 }]; // 20% chance to drop a shard
    this.tickRate = 5; // Ticks randomly based on random tick
    this.growthChance = 0.15; // 15% chance to grow to next stage when ticked
    this.maxGrowthStage = 3; // How many stages total (small -> medium -> large -> cluster)
    this.currentGrowthStage = 0; // 0-based index
  }
  ...
  onRandomTick(world, pos) {
    ...
    if (attachedBlock && attachedBlock.type === 'budding_amethyst') {
      // Grow to the next stage: medium amethyst bud
      world.setBlock(pos, 'medium_amethyst_bud', { facing: this.state.facing, waterlogged: this.state.waterlogged });
    }
  }
...
module.exports = SmallAmethystBud; 
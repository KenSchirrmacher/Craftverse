/**
 * MediumAmethystBud - Second growth stage of amethyst crystals
...
class MediumAmethystBud extends Block {
  /**
   * Create a new MediumAmethystBud block
   * @param {Object} options - Block options
   */
  constructor(options = {}) {
    super(options);
    this.type = 'medium_amethyst_bud';
    ...
    this.drops = [{ id: 'amethyst_shard', count: 1, probability: 0.3 }]; // 30% chance to drop a shard
    this.tickRate = 5; // Ticks randomly based on random tick
    this.growthChance = 0.13; // 13% chance to grow to next stage when ticked
    this.currentGrowthStage = 1; // 0-based index (medium is stage 1)
  }
  ...
  onRandomTick(world, pos) {
    ...
    if (attachedBlock && attachedBlock.type === 'budding_amethyst') {
      // Grow to the next stage: large amethyst bud
      world.setBlock(pos, 'large_amethyst_bud', { facing: this.state.facing, waterlogged: this.state.waterlogged });
    }
  }
...
module.exports = MediumAmethystBud; 